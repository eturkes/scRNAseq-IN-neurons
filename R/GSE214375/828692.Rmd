---
title: "scRNAseq IN Neuron Analysis - GSE214375 - 828692"
author:
  - name: "Emir Turkes"
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    ...
    output_file = file.path("..", "..", "results", unlist(strsplit(getwd(), "/"))[6], "828692.html")
  )})
---
...
```{r}
data <- t(readMM(file.path(data_dir, paste0(sample_name, "_DGE.mtx"))))
meta <- read.csv(file.path(data_dir, paste0(sample_name, "_cell_metadata.csv")))
genes <- read.csv(file.path(data_dir, paste0(sample_name, "_genes.csv")))

colnames(data) <- meta$cell_barcode
rownames(data) <- genes$gene_name
...
remove <- which(duplicated(rownames(sce)))
sce <- sce[-remove, ]
remove <- which(rownames(sce) == "")
sce <- sce[-remove, ]
...
seurat <- cluster_pipeline(
  ...
  vars_to_regress = vars_to_regress, parallel_override = parallel_override, cc = FALSE, res_divider = 1000
)
seurat
