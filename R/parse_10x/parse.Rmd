---
title: "scRNAseq IN Neuron Analysis - Parse Data - Mouse Cortex Nuclei"
author:
  - name: "Emir Turkes"
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    ...
    output_file = file.path("..", "..", "results", unlist(strsplit(getwd(), "/"))[6], "parse.html")
  )})
---
...
```{r}
data <- t(readMM(file.path(data_dir, "DGE.mtx")))
meta <- read.csv(file.path(data_dir, "cell_metadata.csv"))
genes <- read.csv(file.path(data_dir, "all_genes.csv"))

colnames(data) <- meta$bc_wells
...
rm(sce)
meta <- meta[meta$bc_wells %in% colnames(seurat), ]
seurat$sample <- meta$sample
