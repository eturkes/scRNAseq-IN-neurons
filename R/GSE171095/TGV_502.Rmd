---
title: "scRNAseq IN Neuron Analysis - GSE171095 - TGV_502"
author:
  - name: "Emir Turkes"
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    ...
    output_file = file.path("..", "..", "results", unlist(strsplit(getwd(), "/"))[6], "TGV_502.html")
  )})
---
...
```{r}
data <- read.csv(file.path(data_dir, paste0("GSM6093733_", sample_name, "dr", ".csv"))) # dr = doublets removed.

# Move genes from first column to rownames.
# -----------------------------------------
...
We view the cells that would be removed by adaptive thresholds for being an outlier by more than 3 MADs.
However, we do not discard them as QC had been previously performed on this dataset.
...
seurat <- cluster_pipeline(
  ...
  vars_to_regress = vars_to_regress, parallel_override = parallel_override, cc = FALSE, res_divider = 40000
)
seurat
