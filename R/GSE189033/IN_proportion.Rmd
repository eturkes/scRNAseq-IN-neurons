...
sub_files[[1]] <- subset(files[[1]], idents = c("9", "8", "6"))
sub_files[[2]] <- subset(files[[2]], idents = c("17", "19", "13", "3", "6"))
sub_files[[3]] <- subset(files[[3]], idents = c("8", "9", "1", "15", "17", "16", "7", "2", "11"))
sub_files[[4]] <- subset(files[[4]], idents = c("5", "4", "14", "1", "8", "9", "7", "17"))
sub_files[[5]] <- subset(files[[5]], idents = c("4", "10", "8", "11", "1", "5"))
sub_files[[6]] <- subset(files[[6]], idents = c("4", "0", "16", "13"))
sub_files[[7]] <- subset(files[[7]], idents = c("5", "0"))
sub_files[[8]] <- subset(files[[8]], idents = c("18", "2", "13", "20", "0", "11", "6"))
# ---------------------
...
pvalb[[1]] <- subset(files[[2]], idents = "13")
# ---------------------
...
# -----------------------------------------------------------------
proportion <- data.frame(
  "Other_Cells" = c(ncol(files[[2]]), ncol(sub_files[[2]])), "Pvalb_Cells" = rep(ncol(pvalb[[1]]), times = 2)
)
...
# ---------------------
pvalb <- vector("list", length = 1)
pvalb[[1]] <- subset(files[[5]], idents = "11")
# ---------------------
...
for (i in seq_along(pvalb)) {
  print(
    FeaturePlot(
      pvalb[[i]], features = c("Gad1", "Gad2", "Pvalb", "Sst"), order = TRUE, cols = c("lightgrey", "red")
    ) + plot_annotation("D-5", theme = theme(plot.title = element_text(hjust = 0.5, size = 18, face = "bold")))
  )
}
```
...
# -----------------------------------------------------------------
proportion <- data.frame(
  "Other_Cells" = c(ncol(files[[5]]), ncol(sub_files[[5]])), "Pvalb_Cells" = rep(ncol(pvalb[[1]]), times = 2)
)
proportion$IN_Percentage <- round((proportion$Pvalb_Cells / proportion$Other_Cells) * 100)
rownames(proportion) <- c("Out_of_All_Cells", "Out_of_IN_Cells")
datatable_download(proportion)
...
# ---------------------
pvalb <- vector("list", length = 1)
pvalb[[1]] <- subset(files[[6]], idents = "13")
# ---------------------
...
# -----------------------------------------------------------------
proportion <- data.frame(
  "Other_Cells" = c(ncol(files[[6]]), ncol(sub_files[[6]])), "Pvalb_Cells" = rep(ncol(pvalb[[1]]), times = 2)
)
proportion$IN_Percentage <- round((proportion$Pvalb_Cells / proportion$Other_Cells) * 100)
rownames(proportion) <- c("Out_of_All_Cells", "Out_of_IN_Cells")
datatable_download(proportion)
